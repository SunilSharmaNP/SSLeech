...
# Additional Video Tools Configuration
VIDTOOLS_FAST_MODE = environ.get("VIDTOOLS_FAST_MODE", "true").lower() == "true"

WATERMARK_DEFAULT_OPACITY = environ.get("WATERMARK_DEFAULT_OPACITY", "0.8")
try:
    WATERMARK_DEFAULT_OPACITY = float(WATERMARK_DEFAULT_OPACITY)
except:
    WATERMARK_DEFAULT_OPACITY = 0.8

WATERMARK_DEFAULT_SIZE = environ.get("WATERMARK_DEFAULT_SIZE", "15")
try:
    WATERMARK_DEFAULT_SIZE = int(WATERMARK_DEFAULT_SIZE)
    if WATERMARK_DEFAULT_SIZE < 5 or WATERMARK_DEFAULT_SIZE > 30:
        WATERMARK_DEFAULT_SIZE = 15
except:
    WATERMARK_DEFAULT_SIZE = 15

WATERMARK_DEFAULT_POSITION = environ.get("WATERMARK_DEFAULT_POSITION", "BR")
VALID_POSITIONS = ['TL', 'TR', 'BL', 'BR', 'CENTER']
if WATERMARK_DEFAULT_POSITION not in VALID_POSITIONS:
    WATERMARK_DEFAULT_POSITION = "BR"

EXTRACT_AUDIO_BITRATE = environ.get("EXTRACT_AUDIO_BITRATE", "192k")
if len(EXTRACT_AUDIO_BITRATE) == 0:
    EXTRACT_AUDIO_BITRATE = "192k"

SUBTITLE_FONT_OUTLINE = environ.get("SUBTITLE_FONT_OUTLINE", "0")
try:
    SUBTITLE_FONT_OUTLINE = int(SUBTITLE_FONT_OUTLINE)
    if SUBTITLE_FONT_OUTLINE < 0 or SUBTITLE_FONT_OUTLINE > 5:
        SUBTITLE_FONT_OUTLINE = 0
except:
    SUBTITLE_FONT_OUTLINE = 0

VIDEO_TRIM_MAX_DURATION = environ.get("VIDEO_TRIM_MAX_DURATION", "3600")
try:
    VIDEO_TRIM_MAX_DURATION = int(VIDEO_TRIM_MAX_DURATION)
except:
    VIDEO_TRIM_MAX_DURATION = 3600

MERGE_MAX_VIDEOS = environ.get("MERGE_MAX_VIDEOS", "10")
try:
    MERGE_MAX_VIDEOS = int(MERGE_MAX_VIDEOS)
except:
    MERGE_MAX_VIDEOS = 10
...
config_dict = {
    ...
    "WATERMARK_DEFAULT_OPACITY": WATERMARK_DEFAULT_OPACITY,
    "WATERMARK_DEFAULT_SIZE": WATERMARK_DEFAULT_SIZE,
    "WATERMARK_DEFAULT_POSITION": WATERMARK_DEFAULT_POSITION,
    "EXTRACT_AUDIO_BITRATE": EXTRACT_AUDIO_BITRATE,
    "SUBTITLE_FONT_OUTLINE": SUBTITLE_FONT_OUTLINE,
    "VIDEO_TRIM_MAX_DURATION": VIDEO_TRIM_MAX_DURATION,
    "MERGE_MAX_VIDEOS": MERGE_MAX_VIDEOS,
}
