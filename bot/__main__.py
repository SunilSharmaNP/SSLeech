    ...
    try:
        ...
        if len(message.command) > 1 and message.command[1] == "wzmlx":
            ...
            try:
                ...
                return await sendMessage(message, "❌ Invalid token link.")
        ...
        try:
            ...
            LOGGER.warning(f"[start] update_pm_users failed: {db_err}")
    except Exception as e:
        LOGGER.error(f"[start] DM handler crashed: {e}", exc_info=True)
        try:
            await sendMessage(message, f"⚠️ Error: <code>{e}</code>")
        except Exception:
            pass
