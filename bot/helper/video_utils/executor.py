...
class VidEcxecutor:
    ...
    async def _run_cmd(self, cmd, status='prog'):
        ...
        _, code = await gather(self._progress(status), self.listener.suproc.wait())
        if code == 0:
    ...
    async def _progress(self, status='prog'):
        # Minimal progress loop to update internal progress values
        while True:
            await sleep(0.5)
            if self.listener.suproc is None:
                continue
            if getattr(self.listener.suproc, 'returncode', None) is not None:
                break

    # --- Implementations for actions (extract/convert/trim/merge/etc.) ---
    async def _vid_extract(self):
        ...
        if self._metadata:
            ...
            base_dir, (streams, _), self.size = await gather(self._name_base_dir(main_video, 'Extract', len(file_list) > 1), get_metavideo(main_video), get_path_size(main_video))
    ...
    async def _vid_convert(self):
        ...
        if self._metadata:
            ...
            main_video = file_list[0]
            base_dir, (streams, _), self.size = await gather(self._name_base_dir(main_video, 'Convert', multi), get_metavideo(main_video), get_path_size(main_video))
        self._start_handler(streams)
        await gather(self._send_status(), self.event.wait())
        await self._queue()
        if self.is_cancel:
        ...
        for file in file_list:
            ...
            cmd = [FFMPEG_NAME, '-hide_banner', '-ignore_unknown', '-y', '-i', self.path, '-map', '0:v:0', '-vf', f'scale={self._qual[self.data]}:-2', '-map', '0:a:?', '-map', '0:s:?', '-c:a', 'copy', '-c:s', 'copy', self.outfile]
    ...
    async def _rm_stream(self):
        ...
        if self._metadata:
            ...
            main_video = file_list[0]
            base_dir, (streams, _), self.size = await gather(self._name_base_dir(main_video, 'Remove', multi), get_metavideo(main_video), get_path_size(main_video))
        self._start_handler(streams)
        await gather(self._send_status(), self.event.wait())
        await self._queue()
        if self.is_cancel:
    ...
    async def _vid_trimmer(self, start_time, end_time):
        ...
        for file in (file_list := await self._get_files()):
            ...
            self.outfile = ospath.join(base_dir, self.name)
            self._files.append(self.path)
            cmd = [FFMPEG_NAME, '-hide_banner', '-ignore_unknown', '-i', self.path, '-ss', start_time, '-to', end_time, '-map', '0:v:0?', '-map', '0:a:?', '-map', '0:s:?', '-c:v', 'copy', '-c:a', 'copy', '-c:s', 'copy', self.outfile, '-y']
            await self._run_cmd(cmd)
            if self.is_cancel:
                return
        return await self._final_path()
    ...
    async def _vid_compress(self, quality=None):
        ...
        if self._metadata:
            ...
            main_video = file_list[0]
            base_dir, (streams, _), self.size = await gather(self._name_base_dir(main_video, 'Compress', multi), get_metavideo(main_video), get_path_size(main_video))
        self._start_handler(streams)
        await gather(self._send_status(), self.event.wait())
        await self._queue()
        if self.is_cancel:
        ...
        for file in file_list:
            ...
            self.outfile = ospath.join(base_dir, self.name)
            self._files.append(self.path)
            cmd = [FFMPEG_NAME, '-hide_banner', '-ignore_unknown', '-y', '-i', self.path, '-preset', config_dict.get('LIB265_PRESET', 'medium'), '-c:v', 'libx265', '-pix_fmt', 'yuv420p10le', '-crf', '24', '-profile:v', 'main10', '-map', f'0:{self.data["video"]}', '-map', '0:s:?', '-c:s', 'copy']
            if banner := config_dict.get('COMPRESS_BANNER'):
                sub_file = ospath.join(base_dir, 'subtitle.srt')
                self._files.append(sub_file)
                quality_str = f',scale={self._qual[quality]}:-2' if quality else ''
                async with aiopen(sub_file, 'w') as f:
                    await f.write(f'1\n00:00:03,000 --> 00:00:08,00\n{banner}')
                cmd.extend(('-vf', f"subtitles='{sub_file}'{quality_str},unsharp,eq=contrast=1.07", '-metadata', f'title={banner}', '-metadata:s:v', f'title={banner}', '-x265-params', 'no-info=1', '-bsf:v', 'filter_units=remove_types=6'))
            elif quality:
                cmd.extend(('-vf', f'scale={self._qual[quality]}:-2'))
            cmd.extend(('-c:a', 'aac', '-b:a', '160k', '-map', f'0:{self.data.get("audio")}?', self.outfile) if self.data else [self.outfile])
            await self._run_cmd(cmd)
            if self.is_cancel:
                return
        return await self._final_path()
    ...
    async def _vid_marker(self, **kwargs):
        ...
        for file in (file_list := await self._get_files()):
            ...
            if hardusb and await aiopath.exists(subfile):
                fontname = kwargs.get('fontname', '').replace('_', ' ') or config_dict.get('HARDSUB_FONT_NAME')
                fontsize = f',FontSize={fontsize}' if (fontsize := kwargs.get('fontsize') or config_dict.get('HARDSUB_FONT_SIZE')) else ''
            ...
            cmd = [FFMPEG_NAME, '-hide_banner', '-ignore_unknown', '-y', '-i', self.path, '-i', wmpath, '-filter_complex', f"[1][0]scale2ref=w='iw*{wmsize}/100':h='ow/mdar'[wm][vid];[vid][wm]overlay={wmposition}{popupwm}{quality}{hardusb}"]
            if config_dict.get('VIDTOOLS_FAST_MODE'):
                cmd.extend(('-c:v', 'libx264', '-preset', config_dict.get('LIB264_PRESET', 'medium'), '-crf', '25'))
    ...
    async def _merge_subs(self, **kwargs):
        ...
        if len(self._files) > 1:
            ...
            if kwargs.get('hardsub'):
                ...
                if config_dict.get('VIDTOOLS_FAST_MODE'):
                    cmd.extend(('-preset', config_dict.get('LIB264_PRESET', 'medium'), '-c:v', 'libx264', '-crf', '24'))
