...
async def _send_with_blockquoted_preview(message, text, hero_photo):
    ...
    hidden link appended at the very end — no visible blockquote wraps it
    (that used to leave a stray empty quote box above the photo), so only
    the actual image preview shows up, seamlessly under the text.
...
async def poster_cmd(_, message):
    ...
    try:
        ...
        # link-preview thumbnail pinned via link_preview_options — no extra
        # visible box around it, just the text followed by the image.
